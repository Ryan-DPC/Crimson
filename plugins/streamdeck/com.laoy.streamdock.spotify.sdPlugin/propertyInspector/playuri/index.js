...
const $propEvent = {
    ...
    didReceiveSettings(data) {
        ...
        // 恢复已保存的设置
        if ($settings.uri) {
            spotifyUri.value = $settings.uri;
        }
        if ($settings.playOption) {
            playOption.value = $settings.playOption;
        }
    },
...
// 添加URI输入框的监听
spotifyUri.addEventListener('input', function () {
    $settings.uri = spotifyUri.value;
});

// 添加播放选项的监听
playOption.addEventListener('change', function () {
    $settings.playOption = playOption.value;
});
