...
pub fn init_db(pool: &DbPool) -> Result<(), r2d2::Error> {
    ...
    // Migration check: check if the table exists and if it doesn't contain 'opponent'
    let table_exists: bool = conn.query_row(
        "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='rune_cache'",
        [],
        |row| row.get(0),
    ).unwrap_or(0) > 0;

    let has_opponent: bool = conn.query_row(
        "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='rune_cache' AND sql LIKE '%opponent%'",
        [],
        |row| row.get(0),
    ).unwrap_or(0) > 0;

    if table_exists && !has_opponent {
        let _ = conn.execute("DROP TABLE rune_cache", []);
    }
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS rune_cache (
            ...
            patch TEXT,
            timestamp INTEGER,
            PRIMARY KEY (champion, role, build_index, opponent)
        )",
        [],
...
pub fn get_cached_runes(pool: &DbPool, champion: &str, role: &str, patch: &str, opponent: &str) -> Result<HashMap<i32, String>, String> {
    let conn = pool.get().map_err(|e| e.to_string())?;
    
    let mut stmt = conn.prepare("SELECT build_index, data FROM rune_cache WHERE champion = ?1 AND role = ?2 AND patch = ?3 AND opponent = ?4")
        .map_err(|e| e.to_string())?;
    let rows = stmt.query_map(params![champion.to_lowercase(), role, patch, opponent.to_lowercase()], |row| {
...
pub fn save_rune_cache(pool: &DbPool, champion: &str, role: &str, patch: &str, build_index: i32, opponent: &str, data: &str) -> Result<(), String> {
    let conn = pool.get().map_err(|e| e.to_string())?;
    
    conn.execute(
        "INSERT OR REPLACE INTO rune_cache (champion, role, build_index, opponent, data, patch, timestamp)
         VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
        params![
            champion.to_lowercase(),
            role,
            build_index,
            opponent.to_lowercase(),
            data,
            patch,
