...
- `tools/plugin_runtime_tester` exercises the usable plugins (Spotify, Discord, LoL) plus Hue/Twitch gating against a running sidecar. With the server up (`CRIMSON_DEV=1`, `APPDATA` set), from `tools/plugin_runtime_tester`: `npm install && CRIMSON_TEST_EMAIL=... CRIMSON_TEST_PASSWORD=... node tester.mjs`. StreamDock hardware events must use `keyDown` (not `keyUp`).
